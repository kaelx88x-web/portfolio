...
<PageHeader title="Concentration Advisor" subtitle="Single-name, asset-class, and currency concentration warning intelligence." breadcrumb={[{ label: 'AI', href: '/ai' }, { label: 'Risk Advisor', href: '/ai/risk-advisor' }, { label: 'Concentration' }]} />
{#if form?.answer}<div class="answer"><strong>{form.answer.title}</strong><span>{form.answer.summary}</span></div>{/if}
<div class="layout">
  <main><RiskAdvisorPanel response={data.concentration} type="concentration" /><ConcentrationRiskTable rows={data.concentrationTable} /></main>
  <aside>{#each data.alerts as alert}<RiskAlertCard {alert} />{/each}</aside>
</div>
